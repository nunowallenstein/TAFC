...
if docsvtoroot:
    ...
    #preprocessing of the data, applying a PCA with 4 components and renormalizing the data with z=(x-\mu)/(\sigma)
    for filename in ["proctrain.csv","proctest.csv"]:
...
# training is the most simple BDTG in TMVA
...
if dotraining:
    ...
    if debug:
        print "all input variables to BDTG", mva_input_list
       # print "all input variables to Fisher ", mva_input_list
    ...
    # only one input tree
    dataloader.SetInputTrees(traintree, signalCut, backgrCut );
    ...
    for var in mva_input_list:
        if var=="PRI_jet_num":
            dataloader.AddVariable( var, var, "units", 'I' );
        else:
            dataloader.AddVariable( var, var, "units", 'D' );
        n +=1
    ...
    dataloader.PrepareTrainingAndTestTree( signalCut, backgrCut,"nTrain_Signal=0:nTrain_Background=0:nTest_Signal=1:nTest_Background=1:SplitMode=Block")
    
    
    method =  factory.BookMethod( dataloader, TMVA.Types.kBDT, "BDTG", "!H:!V:NTrees=800:MinNodeSize=5%:BoostType=Grad:Shrinkage=0.15:UseBaggedBoost:BaggedSampleFraction=0.7:nCuts=-1:MaxDepth=2:SeparationType=MisClassificationError:PruneMethod=CostComplexity:UseBaggedGrad=True")
    #BDTG1
   # method =  factory.BookMethod( dataloader, TMVA.Types.kBDT, "BDTG", "!H:!V:NTrees=500:MinNodeSize=5%:BoostType=Grad:Shrinkage=1:UseBaggedBoost:BaggedSampleFraction=0.5:nCuts=30:MaxDepth=2:SeparationType=MisClassificationError")
   #Best amsfinal  3.17533566361  for threshold : 0.85  ( ams simple = 3.19551456409 , ams asimov= 3.17720482075 )
   #Score=3.12853
   
   #BDTG2
   #method =  factory.BookMethod( dataloader, TMVA.Types.kBDT, "BDTG", "!H:!V:NTrees=800:MinNodeSize=5%:BoostType=Grad:Shrinkage=0.15:UseBaggedBoost:BaggedSampleFraction=0.7:nCuts=-1:MaxDepth=2:SeparationType=MisClassificationError:PruneMethod=CostComplexity:UseBaggedGrad=True")
#Best amsfinal  3.53347983596  for threshold : 0.862  ( ams simple = 3.56666156239 , ams asimov= 3.5370194165 )
  #Score= 3.43621


   #method 
   # method=factory.BookMethod( dataloader, TMVA.Types.kFisher, "Fisher", "H:!V:Fisher:VarTransform=None:CreateMVAPdfs:PDFInterpolMVAPdf=Spline2:NbinsMVAPdf=50:NsmoothMVAPdf=10" );
#score=2.13
    # launch the training
...
if doevaluate:
    ...
    for name in ["proctrain","proctest"]:
        inputfilename=name+".csv.root"
        outputfilename=name+"_score.root"

        new_variables_list = [ 'bdtg', ]
        #new_variables_list = [ 'Fisher', ]
        ...
        reader=TMVA.Reader()
        # Add to the reader the variables to be used for bdtg evaluation (only that one)
        ...
        for i in xrange(0, max_index):
            ...
            # actually this is not be necessary for variables that are not used by BDTG
            for var in varlist:
                exec ('maps["'+var+'"][0]=inputtree.'+var)

            # compute bdtg scoree    
            maps['bdtg'][0]=reader.EvaluateMVA("BDTG method")
            # compute Fisher scoree    
            #maps['Fisher'][0]=reader.EvaluateMVA("Fisher method")
...
if dothreshold:
    ...
    inputfilename="proctrain_score.root"
    inputfile = TFile.Open(inputfilename,"read")
    inputtree    = inputfile.Get(treename)
    ...
    scoremin=-1
    scoremax=1
    hsig=TH1F("hsig","signal bdtg score",nbins,scoremin,scoremax)
    hbkg=TH1F("hbkg","background bdtg score",nbins,scoremin,scoremax)    
    ...
    #inputtree.Project("hsig","Fisher","Weight*(Label==1)")
    
    #inputtree.Project("hbkg","Fisher","Weight*(Label==0)")

    inputtree.Project("hsig","bdtg","Weight*(Label==1)")
    
    inputtree.Project("hbkg","bdtg","Weight*(Label==0)")
        # copy into vectors
    ...
    outputfile2 = TFile.Open("teste.root", 'RECREATE')
    hsigint=TH1F("hsigint","integrated signal vs bdtg score",nbins,scoremin,scoremax)
    hbkgint=TH1F("hbkgint","integrated bkg vs bdtg score",nbins,scoremin,scoremax)        
    hamsfinal=TH1F("hamsfinal","final ams vs bdtg score",nbins,scoremin,scoremax)
    hamsasimov=TH1F("hamsasimov","asimov ams vs bdtg score",nbins,scoremin,scoremax)
    hamssimple=TH1F("hamssimple","simple ams vs bdtg score",nbins,scoremin,scoremax)
...
if dosubmission:
    ...
    inputfilename="proctest_score.root"
    ...
    print  "Loop once to load the EventId, bdtg score pairs"
    max_index = inputtree.GetEntries()
    allentries=[]
    for i in xrange(0, max_index):
        if i % 100000==1:
            print " processing event ",i 
        inputtree.GetEntry(i)
        allentries+=[(inputtree.EventId,inputtree.bdtg)]
        #allentries+=[(inputtree.EventId,inputtree.Fisher)]

    # sort on the Fisher,
    # print "sort on the Fisher score"
    print "sort on the bdtg score"
    ...
    for i in xrange(0, max_index):
        ...
        # compute label 
        slabel="b"
        if inputtree.bdtg>threshold:
        #if inputtree.Fisher>threshold:
            slabel="s"
