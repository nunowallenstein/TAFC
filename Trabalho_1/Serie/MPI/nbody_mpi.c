...
void advance_particles(double **param,const int N_particles,const double dt,int world_rank, int world_size)
{
 ...
 double *vx_total=calloc(N_particles,sizeof(double));
  double *vy_total=calloc(N_particles,sizeof(double));
  double *vz_total=calloc(N_particles,sizeof(double));
  */
  ...
  for(int i =0;i<N_particles;i++)
	...
	{
	  double dx=param[0][j]-param[0][i];
	  double dy=param[1][j]-param[1][i];
	  double dz=param[2][j]-param[2][i];
      ...
      */
      ...
      // MPI_Barrier(MPI_COMM_WORLD);
         param[3][i]=param[3][i]+ Fx * dt;
      param[4][i]= param[4][i]+ Fy * dt;
       param[5][i]=param[5][i]+ Fz * dt;
 ...
 tmpx=param[0][i];
      ...
      param[0][i]=tmpx + param[3][i]*dt;
      param[1][i]=tmpy+ param[4][i] * dt;
      param[2][i] =tmpz+ param[5][i]* dt;
 ...
 MPI_Bcast(&param[2][i],1, MPI_DOUBLE, world_rank, MPI_COMM_WORLD);
  ...
  /*  MPI_Allreduce(&(param[0]),&(x_total[0]),N_particles,MPI_DOUBLE,MPI_SUM,MPI_COMM_WORLD);
   MPI_Allreduce(&(param[1]),&(y_total[0]),N_particles,MPI_DOUBLE,MPI_SUM,MPI_COMM_WORLD);
   MPI_Allreduce(&(param[2]),&(z_total[0]),N_particles,MPI_DOUBLE,MPI_SUM,MPI_COMM_WORLD);
  */
